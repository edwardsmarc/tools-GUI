...
GUI_v3 <- function(){
  ...
  source("../functions/metricsDistanceMean.R")
  source("../functions/metricsRepresentation.R")
  source("../functions/metricsColumnSum.R")
  source("../functions/metricsSI.R")
  source("../functions/metricsOverlap.R")
  source("../functions/metricsHoles.R")
  source("../functions/metricsHydro.R")
  ...
  nb10.2 <- ggroup(container = nb10_nb, label = "Representation targets", horizontal = FALSE)
  nb10.3 <- ggroup(container = nb10_nb, label = "Shape index", horizontal = FALSE)
  nb10.4 <- ggroup(container = nb10_nb, label = "Overlap", horizontal = FALSE)
  nb10.5 <- ggroup(container = nb10_nb, label = "Holes", horizontal = FALSE)
  nb10.6 <- ggroup(container = nb10_nb, label = "Upstream hydrology", horizontal = FALSE)
  ##
  ...
  lyt3a[11,1:2] <- gp_3_run_btn <- gbutton(
    ...
    handler = function(h, ...){ 
      ...
      if(svalue(gp_3_newShp_check) == TRUE){ 
        if(length(svalue(gp_3_outFile_btn)) == 0){stop("Output file needs to be provided if box is checked")}
...
and multipled by the area target to get the representation target in km2."))
  ...
  lyt5[1:15, 1:2] <- gp_5_frm_1 <- gframe("Assess benchmark representation", container = lyt5, spacing = 10, expand = TRUE)
  lyt5a <- glayout(container = gp_5_frm_1)
  
  # browse to select target table
  lyt5a[2,1] <- "Target table (.csv)"
  lyt5a[2,2] <- gp_5_target_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt5a, 
  ...
  # text box to input criteria
  lyt5a[3,1] <- "Criteria name (e.g. cmi)"
  lyt5a[3,2] <- gp_5_criteria_txt <- gedit(text = "", 
                                           container = lyt5a,
                                           handler = function(h,...){
  ...
  lyt5a[4,1] <- "Output benchmarks results table (.csv)"
  lyt5a[4,2] <- gp_5_outFile_btn <- gfilebrowse( # file browser
    text = "", 
    type = "save", 
    quote = TRUE, 
    container = lyt5a, 
  ...
  lyt5a[5,1:2] <- gseparator(horizontal = TRUE, container = lyt5a, expand=TRUE)
  
  # browse to select catchments dbf file
  lyt5a[6,1] <- "Catchments file (.dbf; optional)"
  lyt5a[6,2] <- gp_5_catchDbf_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt5a, 
  ...
  # browse to select builder csv file
  lyt5a[7,1] <- "Builder ranker file (.csv; optional)"
  lyt5a[7,2] <- gp_5_buildCsv_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt5a, 
  ...
  lyt5a[8,1] <- "Filter file (.csv; optional)"
  lyt5a[8,2] <- gp_5_Filter_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt5a, 
  ...
  lyt5a[9,1:2] <- gseparator(horizontal = TRUE, container = lyt5a, expand=TRUE)
  
  # browse to select optional shp file
  lyt5a[10,1] <- "PB shapefile (.shp; optional)"
  lyt5a[10,2] <- gp_5_shp_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt5a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "PB shapefile: ", svalue(gp_5_shp_btn))
    }
  )
  
  # browse to select optional raster clip file
  lyt5a[11,1] <- "Criteria raster (.asc; optional)"
  lyt5a[11,2] <- gp_5_ras_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt5a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Criteria raster: ", svalue(gp_5_ras_btn))
    }
  )
  
  # text box to input cell size
  lyt5a[12,1] <- "Cell size (metres; optional)"
  lyt5a[12,2] <- gp_5_cellsize_txt <- gedit(text = "", 
                                           container = lyt5a,
                                           handler = function(h,...){
                                             svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Cell size (metres): ", svalue(gp_5_cellsize_txt))
                                           })
  
  # text box to input disaggregate factor
  lyt5a[13,1] <- "Disaggregate factor value (optional)"
  lyt5a[13,2] <- gp_5_factor_txt <- gedit(text = "1", 
                                         container = lyt5a,
                                         handler = function(h,...){
                                           svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Disaggregate factor value: ", svalue(gp_5_factor_txt))
                                         })
  
  lyt5a[14,1:2] <- gseparator(horizontal = TRUE, container = lyt5a, expand=TRUE)
  
  # button to run the benchmarkRep function
  lyt5a[15,1:2] <- gp_5_run_btn <- gbutton(
    text = "Assess benchmark representation",
    container = lyt5a,
    handler = function(h, ...){ 
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Working...")
      
      if(length(svalue(gp_5_catchDbf_btn)) == 0){ catch5 <- ""} else{ catch5 <- svalue(gp_5_catchDbf_btn)}
      
      if(length(svalue(gp_5_buildCsv_btn)) == 0){ build5 <- ""} else{ build5 <- svalue(gp_5_buildCsv_btn)}
      
      if(length(svalue(gp_5_shp_btn)) == 0){ shp5 <- ""} else{ shp5 <- svalue(gp_5_shp_btn)}
      
      if(length(svalue(gp_5_ras_btn)) == 0){ rast5 <- ""} else{ rast5 <- svalue(gp_5_ras_btn)}
      
      if(length(svalue(gp_5_cellsize_txt)) == 0){ cell5 <- ""} else{ cell5 <- svalue(gp_5_cellsize_txt)}
      
      if(length(svalue(gp_5_factor_txt)) == 0){ factor5 <- ""} else{ factor5 <- svalue(gp_5_factor_txt)}
      
      if(length(svalue(gp_5_Filter_btn)) == 0){ baFilter5 <- ""} else{ baFilter5 <- svalue(gp_5_Filter_btn)}
  ...
  lyt5[1:15, 3] <- gp_5_text <- gframe("Tool description", container = lyt5, expand = TRUE, pos = 1)
...
table."))
  ...
  lyt7[1:12, 1:2] <- gp_7_frm_1 <- gframe("Summarise target based representation results", container = lyt7, spacing = 10, expand = TRUE)
  ...
  # text box to input gap threshold value
  lyt7a[8,1] <- "Pass threshold for target  (%)"
  lyt7a[8,2] <- gp_7_gapThreshold_txt <- gedit(text = "100", 
                         container = lyt7a,
                         handler = function(h,...){
                           svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Pass threshold for target: ", svalue(gp_7_gapThreshold_txt))
                         })
  
  # browse to select output csv file
  lyt7a[10,1] <- "Output summary results table (.csv)"
  lyt7a[10,2] <- gp_7_outFile_btn <- gfilebrowse( # file browser
    text = "", 
    type = "save", 
    quote = TRUE, 
    container = lyt7a, 
  ...
  lyt7a[12,1:2] <- gp_7_run_btn <- gbutton(
    ...
    handler = function(h, ...){ 
      tryCatch({
        results_summary(
          ...
          gapThreshold = svalue(gp_7_gapThreshold_txt),
  ...
  lyt7[1:12, 3] <- gp_7_text <- gframe("Tool description", container = lyt7, expand = TRUE, pos = 1)
...
requirements for benchmark status."))
  ...
  # Framing - glayout window contining three gframes. Each frame contains another layout grid for placing widgets. 
  # Frame 1 - widgets to run metricsDissimilarity.R. Columns 1:2.
  # Frame 2 - widgets to run dissimilarity_distance_mean.R. Columns 1:2.
  # Frame 3 - text describing inputs. Column 3.
  
  lyt10.1 <- glayout(container = nb10.1)
  
  lyt10.1[1:11, 1:2] <- gp_10.1_frm_1 <- gframe("Calculate dissimilarity values", container = lyt10.1, spacing = 10, expand = TRUE)
  ...
  # check box and browse for optional plot directory
  lyt10.1a[10,1] <- gp_10.1_plotDir_check <- gcheckbox(text = "Make plots? (output folder; optional)", checked=FALSE, container = lyt10.1a)
  
  # browser for output shapefile
  lyt10.1a[10,2] <- gp_10.1_plotDir_btn <- gfilebrowse( # file browser
    text = "", 
    type = "selectdir", 
    quote = TRUE, 
    container = lyt10.1a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Output plot folder: ", svalue(gp_10.1_plotDir_btn))
    }
  )
  
  # button to run the metricsDissimilarity function
  lyt10.1a[11,1:2] <- gp_10.1_run_btn <- gbutton(
    ...
    handler = function(h, ...){ 
      ...
      if(svalue(gp_10.1_plotDir_check) == TRUE){
        if(length(svalue(gp_10.1_plotDir_btn)) == 0){stop("Output file path needs to be provided if box is checked.")}
        plotDirectory <- svalue(gp_10.1_plotDir_btn)
      } else{
        plotDirectory <- ""
      }
      tryCatch({
  ...
  lyt10.1[13:15, 1:2] <- gp_10.1_frm_2 <- gframe("Summarise dissimilarity", container = lyt10.1, spacing = 10, expand = TRUE)
  lyt10.1ai <- glayout(container = gp_10.1_frm_2)
  
  # Checkboxes for euclidean distance and mean
  lyt10.1ai[1,1] <- gp_10.1ai_dist_check <- gcheckbox(text = "Euclidean distance", checked=FALSE, container = lyt10.1ai)
  lyt10.1ai[1,2] <- gp_10.1ai_mean_check <- gcheckbox(text = "Mean dissimilarity", checked=FALSE, container = lyt10.1ai)
  
  # text input for column names to summarise
  lyt10.1ai[2,1] <- "Columns (e.g. ks_cmi,ks_led,ks_gpp,bc_lcc)"
  lyt10.1ai[2,2] <- gp_10.1ai_colNames_txt <- gedit(text = "", 
                                               container = lyt10.1ai,
                                               handler = function(h,...){
                                                 svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Columns names: ", svalue(gp_10.1ai_colNames_txt))
                                                 })
  
  # button to run function
  lyt10.1ai[3,1:2] <- gp_10.1ai_run_btn <- gbutton(
    text = "Add dissimilarity summary columns",
    container = lyt10.1ai,
    handler = function(h, ...){ 
      tryCatch({
        dissimilarity_distance_mean(
          netPath = svalue(gp_10.1_inshp_btn), 
          colNames = svalue(gp_10.1ai_colNames_txt), 
          calcDistance = svalue(gp_10.1ai_dist_check),
          calcMean = svalue(gp_10.1ai_mean_check)
        )
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Dissimilarity summary complete")
      }, error = function(cond){
        message("Could not summarise dissimilarity")
        message(cond)
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Could not summarise dissimilarity")
      }
      )
    }
  )
  
  lyt10.1[1:11, 3] <- gp_10.1_text <- gframe("Tool description", container = lyt10.1, expand = TRUE, pos = 1)
...
option. The name should not contain any spaces."))
  ...
  ###### tab 10.2 - Add representation results to network shapefiles ######################################
  # Framing - glayout window contining three gframes. Each frame contains another layout grid for placing widgets. 
  # Frame 1 - widgets to run metricsRepresentation.R. Columns 1:2.
  # Frame 2 - widgets to run metricsColumnSum.R. Columns 1:2.
  # Frame 3 - text describing inputs. Column 3.
  
  lyt10.2 <- glayout(container = nb10.2)
  
  lyt10.2[1:8, 1:2] <- gp_10.2_frm_1 <- gframe("Summarise representation results", container = lyt10.2, spacing = 10, expand = TRUE)
  lyt10.2a <- glayout(container = gp_10.2_frm_1)
  
  # browse to select input shapefile
  lyt10.2a[2,1] <- "Network file (.shp)"
  lyt10.2a[2,2] <- gp_10.2_inshp_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.2a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Network shapefile: ", svalue(gp_10.2_inshp_btn))
    }
  )
  
  # browse to select results csv
  lyt10.2a[3,1] <- "Representation results table (.csv)"
  lyt10.2a[3,2] <- gp_10.2_rep_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.2a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Representation results table: ", svalue(gp_10.2_rep_btn))
    }
  )
  
  # text box to input threshold value
  lyt10.2a[4,1] <- "Threshold value for class inclusion (%)"
  lyt10.2a[4,2] <- gp_10.2_threshold_txt <- gedit(text = "0", 
                                            container = lyt10.2a,
                                            handler = function(h,...){
                                              svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Threshold value for class inclusion: ", svalue(gp_10.2_threshold_txt))
                                            })
  
  # text box to input gap threshold value
  lyt10.2a[5,1] <- "Pass threshold for target  (%)"
  lyt10.2a[5,2] <- gp_10.2_gapThreshold_txt <- gedit(text = "100", 
                                               container = lyt10.2a,
                                               handler = function(h,...){
                                                 svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Pass threshold for target: ", svalue(gp_10.2_gapThreshold_txt))
                                               })
  
  # browse to select target table
  lyt10.2a[6,1] <- "Target table (.csv; optional)"
  lyt10.2a[6,2] <- gp_10.2_target_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.2a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Target csv file: ", svalue(gp_10.2_target_btn))
    }
  )
  
  # text box to input new column name
  lyt10.2a[7,1] <- "New column name"
  lyt10.2a[7,2] <- gp_10.2_newCol_txt <- gedit(text = "", 
                                               container = lyt10.2a,
                                               handler = function(h,...){
                                                 svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "New column name: ", svalue(gp_10.2_newCol_txt))
                                                 })
  
  # button to run function
  lyt10.2a[8,1:2] <- gp_10.2_run_btn <- gbutton(
    text = "Add representation summary column",
    container = lyt10.2a,
    handler = function(h, ...){ 
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Working...")
      if(length(svalue(gp_10.2_target_btn)) == 0){ gp10.2_targetTab <- ""} else{ gp10.2_targetTab <- svalue(gp_10.2_target_btn)}
      tryCatch({
        metricsRepresentation(
          netPath = svalue(gp_10.2_inshp_btn), 
          resultsTab = svalue(gp_10.2_rep_btn), 
          rareThreshold = svalue(gp_10.2_threshold_txt),
          gapThreshold = svalue(gp_10.2_gapThreshold_txt),
          targetTable = gp10.2_targetTab,
          newCol = svalue(gp_10.2_newCol_txt)
        )
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Representation summary complete")
      }, error = function(cond){
        message("Could not summarise representation")
        message(cond)
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Could not summarise representation")
      }
      )
    }
  )
  
  lyt10.2[9:12, 1:2] <- gp_10.2_frm_2 <- gframe("Sum gaps", container = lyt10.2, spacing = 10, expand = TRUE)
  lyt10.2ai <- glayout(container = gp_10.2_frm_2)
  
  # text input for column names to sum
  lyt10.2ai[2,1] <- "Columns (e.g. gap_cmi,gap_led,gap_gpp,gap_lcc)"
  lyt10.2ai[2,2] <- gp_10.2ai_colNames_txt <- gedit(text = "", 
                                                    container = lyt10.2ai,
                                                    handler = function(h,...){
                                                      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Columns names: ", svalue(gp_10.2ai_colNames_txt))
                                                    })
  
  # text input for sum column name
  lyt10.2ai[3,1] <- "New column name"
  lyt10.2ai[3,2] <- gp_10.2ai_newCol_txt <- gedit(text = "", 
                                                    container = lyt10.2ai,
                                                    handler = function(h,...){
                                                      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "New column name: ", svalue(gp_10.2ai_newCol_txt))
                                                    })
  
  # button to run function
  lyt10.2ai[4,1:2] <- gp_10.2ai_run_btn <- gbutton(
    text = "Add sum column",
    container = lyt10.2ai,
    handler = function(h, ...){ 
      tryCatch({
        metricsColumnSum(
          netPath = svalue(gp_10.2_inshp_btn), 
          colNames = svalue(gp_10.2ai_colNames_txt), 
          newCol = svalue(gp_10.2ai_newCol_txt)
        )
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Sum column added")
      }, error = function(cond){
        message("Could not add sum column")
        message(cond)
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Could not add sum column")
      }
      )
    }
  )
  
  lyt10.2[1:12, 3] <- gp_10.2_text <- gframe("Tool description", container = lyt10.2, expand = TRUE, pos = 1)
  lyt10.2b <- glayout(container = gp_10.2_text, spacing = 10)
  lyt10.2b[1,1] <- glabel(paste0("Each tool in the 'Add network attributes' tab adds new fields to the networks shapefile.
                                 It is recommended you keep a copy of the original file before adding new attributes.
                                 ----------"))
  
  
  ###### tab 10.3 - Add shape index to network shapefiles ######################################
  # Framing - glayout window contining two gframes. Each frame contains another layout grid for placing widgets. 
  # Frame 1 - widgets to run metricsSI.R. Columns 1:2.
  # Frame 2 - text describing inputs. Column 3.
  
  lyt10.3 <- glayout(container = nb10.3)
  
  lyt10.3[1:4, 1:2] <- gp_10.3_frm_1 <- gframe("Calculate shape index", container = lyt10.3, spacing = 10, expand = TRUE)
  lyt10.3a <- glayout(container = gp_10.3_frm_1)
  
  # browse to select input shapefile
  lyt10.3a[2,1] <- "Network file (.shp)"
  lyt10.3a[2,2] <- gp_10.3_inshp_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.3a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Network shapefile: ", svalue(gp_10.3_inshp_btn))
    }
  )
  
  # text input for sum column name
  lyt10.3a[3,1] <- "New column name"
  lyt10.3a[3,2] <- gp_10.3a_newCol_txt <- gedit(text = "SI", 
                                                  container = lyt10.3a,
                                                  handler = function(h,...){
                                                    svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "New column name: ", svalue(gp_10.3a_newCol_txt))
                                                  })
  
  # button to run function
  lyt10.3a[4,1:2] <- gp_10.3a_run_btn <- gbutton(
    text = "Calculate shape index",
    container = lyt10.3a,
    handler = function(h, ...){ 
      tryCatch({
        metricsSI(
          netPath = svalue(gp_10.3_inshp_btn), 
          newCol = svalue(gp_10.3a_newCol_txt)
        )
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Shape index column added")
      }, error = function(cond){
        message("Could not calculate shape index")
        message(cond)
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Shape index column added")
      }
      )
  ...
  lyt10.3[1:4, 3] <- gp_10.3_text <- gframe("Tool description", container = lyt10.3, expand = TRUE, pos = 1)
  lyt10.3b <- glayout(container = gp_10.3_text, spacing = 10)
  lyt10.3b[1,1] <- glabel(paste0("Each tool in the 'Add network attributes' tab adds new fields to the networks shapefile.
                                 It is recommended you keep a copy of the original file before adding new attributes.
                                 ----------"))


  ###### tab 10.4 - Add overlap to network shapefiles ######################################
  # Framing - glayout window contining two gframes. Each frame contains another layout grid for placing widgets. 
  # Frame 1 - widgets to run metricsOverlap.R. Columns 1:2.
  # Frame 2 - text describing inputs. Column 3.
  
  lyt10.4 <- glayout(container = nb10.4)
  
  lyt10.4[1:4, 1:2] <- gp_10.4_frm_1 <- gframe("Calculate overlap", container = lyt10.4, spacing = 10, expand = TRUE)
  lyt10.4a <- glayout(container = gp_10.4_frm_1)
  
  # browse to select input shapefile
  lyt10.4a[2,1] <- "Network file (.shp)"
  lyt10.4a[2,2] <- gp_10.4_inshp_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.4a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Network shapefile: ", svalue(gp_10.4_inshp_btn))
    }
  )
  
  # browse to select reference shapefile
  lyt10.4a[3,1] <- "Reference file (.shp)"
  lyt10.4a[3,2] <- gp_10.4_refshp_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.4a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Reference shapefile: ", svalue(gp_10.4_refshp_btn))
    }
  )
  
  # text input for overlap column name
  lyt10.4a[4,1] <- "New column name"
  lyt10.4a[4,2] <- gp_10.4a_newCol_txt <- gedit(text = "Overlap", 
                                                container = lyt10.4a,
                                                handler = function(h,...){
                                                  svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "New column name: ", svalue(gp_10.4a_newCol_txt))
                                                })
  
  # button to run function
  lyt10.4a[5,1:2] <- gp_10.4a_run_btn <- gbutton(
    text = "Calculate overlap",
    container = lyt10.4a,
    handler = function(h, ...){ 
      tryCatch({
        metricsOverlap(
          netPath = svalue(gp_10.4_inshp_btn), 
          refPath = svalue(gp_10.4_refshp_btn), 
          newCol = svalue(gp_10.4a_newCol_txt)
        )
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Overlap column added")
      }, error = function(cond){
        message("Could not calculate overlap")
        message(cond)
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Overlap column added")
      }
      )
  ...
  lyt10.4[1:5, 3] <- gp_10.4_text <- gframe("Tool description", container = lyt10.4, expand = TRUE, pos = 1)
  lyt10.4b <- glayout(container = gp_10.4_text, spacing = 10)
  lyt10.4b[1,1] <- glabel(paste0("Each tool in the 'Add network attributes' tab adds new fields to the networks shapefile.
                                 It is recommended you keep a copy of the original file before adding new attributes.
                                 ----------"))
  
  
  ###### tab 10.5 - Add proportion of gaps to network shapefiles ######################################
  # Framing - glayout window contining two gframes. Each frame contains another layout grid for placing widgets. 
  # Frame 1 - widgets to run metricsHoles.R. Columns 1:2.
  # Frame 2 - text describing inputs. Column 3.
  
  lyt10.5 <- glayout(container = nb10.5)
  
  lyt10.5[1:7, 1:2] <- gp_10.5_frm_1 <- gframe("Calculate holes", container = lyt10.5, spacing = 10, expand = TRUE)
  lyt10.5a <- glayout(container = gp_10.5_frm_1)
  
  # browse to select network shapefile
  lyt10.5a[2,1] <- "Network file (.shp)"
  lyt10.5a[2,2] <- gp_10.5_inshp_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.5a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Network shapefile: ", svalue(gp_10.5_inshp_btn))
    }
  )

    # browse to select benchmarks shapefile
  lyt10.5a[3,1] <- "Benchmark file (.shp)"
  lyt10.5a[3,2] <- gp_10.5_bashp_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.5a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Benchmark shapefile: ", svalue(gp_10.5_bashp_btn))
    }
  )
  
  # browse to select builder csv file
  lyt10.5a[4,1] <- "Builder ranker file (.csv)"
  lyt10.5a[4,2] <- gp_10.5_buildCsv_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.5a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Builder csv file: ", svalue(gp_10.5_buildCsv_btn))
    }
  )
  
  # text input for holes column name
  lyt10.5a[5,1] <- "New column name"
  lyt10.5a[5,2] <- gp_10.5_newCol_txt <- gedit(text = "", 
                                                container = lyt10.5a,
                                                handler = function(h,...){
                                                  svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "New column name: ", svalue(gp_10.5_newCol_txt))
                                                })
  
  # browse to select additional catchments file
  lyt10.5a[6,1] <- "Additional benchmark catchments table (.csv; optional)"
  lyt10.5a[6,2] <- gp_10.5_addCatch_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.5a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Additional benchmark catchments csv file: ", svalue(gp_10.5_addCatch_btn))
    }
  )
  
  # button to run function
  lyt10.5a[7,1:2] <- gp_10.5a_run_btn <- gbutton(
    text = "Calculate holes",
    container = lyt10.5a,
    handler = function(h, ...){ 
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Working...")
      if(length(svalue(gp_10.5_buildCsv_btn)) > 0){
        buildCsv_10.5 <- svalue(gp_10.5_buildCsv_btn)
      } else{
        buildCsv_10.5 <- ""
      }
      if(length(svalue(gp_10.5_addCatch_btn)) > 0){
        addCatch_10.5 <- svalue(gp_10.5_addCatch_btn)
      } else{
        addCatch_10.5 <- ""
      }
      tryCatch({
        metricsHoles(
          netPath = svalue(gp_10.5_inshp_btn), 
          idColNet = "networks", 
          baPath = svalue(gp_10.5_bashp_btn),
          idColBa = "PB",
          buildCsv = buildCsv_10.5,
          newCol = svalue(gp_10.5_newCol_txt),
          buildCsvAdd = addCatch_10.5
        )
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Holes proportion added")
      }, error = function(cond){
        message("Could not calculate holes")
        message(cond)
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Could not calculate holes")
      }
      )
    }
  )
  
  lyt10.5[1:7, 3] <- gp_10.5_text <- gframe("Tool description", container = lyt10.5, expand = TRUE, pos = 1)
  lyt10.5b <- glayout(container = gp_10.5_text, spacing = 10)
  lyt10.5b[1,1] <- glabel(paste0("Each tool in the 'Add network attributes' tab adds new fields to the networks shapefile.
                                 It is recommended you keep a copy of the original file before adding new attributes.
                                 ----------"))
  
  
  ###### tab 10.6 - Add upstream area and AWI to network shapefiles ######################################
  # Framing - glayout window contining two gframes. Each frame contains another layout grid for placing widgets. 
  # Frame 1 - widgets to run metricsHoles.R. Columns 1:2.
  # Frame 2 - text describing inputs. Column 3.
  
  lyt10.6 <- glayout(container = nb10.6)
  
  lyt10.6[1:20, 1:2] <- gp_10.6_frm_1 <- gframe("Calculate hydrology", container = lyt10.6, spacing = 10, expand = TRUE)
  lyt10.6a <- glayout(container = gp_10.6_frm_1)
  
  # browse to select network shapefile
  lyt10.6a[2,1] <- "Network file (.shp)"
  lyt10.6a[2,2] <- gp_10.6_inshp_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.6a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Network shapefile: ", svalue(gp_10.6_inshp_btn))
    }
  )
  
  # browse to select catchments dbf file
  lyt10.6a[3,1] <- "Catchments file (.dbf)"
  lyt10.6a[3,2] <- gp_10.6_catchDbf_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.6a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Catchments dbf: ", svalue(gp_10.6_catchDbf_btn))
    }
  )
  
  # text box to input intactness column
  lyt10.6a[4,1] <- "Intactness column"
  lyt10.6a[4,2] <- gp_10.6_intactcol_txt <- gedit(text = "", 
                                           container = lyt10.6a,
                                            handler = function(h,...){
                                              svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Intactness column: ", svalue(gp_10.6_intactcol_txt))
                                            })
  
  # text box to input area column
  lyt10.6a[5,1] <- "Area column"
  lyt10.6a[5,2] <- gp_10.6_areacol_txt <- gedit(text = "", 
                                           container = lyt10.6a,
                                            handler = function(h,...){
                                              svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Area column: ", svalue(gp_10.6_areacol_txt))
                                            })
  
  # text input for hydro columns column name
  lyt10.6a[6,1] <- "New column name"
  lyt10.6a[6,2] <- gp_10.6_newCol_txt <- gedit(text = "", 
                                               container = lyt10.6a,
                                               handler = function(h,...){
                                                 svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "New column name: ", svalue(gp_10.6_newCol_txt))
                                               })
  
  lyt10.6a[7,1:2] <- gseparator(horizontal = TRUE, container = lyt10.6a, expand=TRUE)
  
  # browse to select builder upstream csv file
  lyt10.6a[8,1] <- "Builder upstream/downstream COLUMNS file (.csv)"
  lyt10.6a[8,2] <- gp_10.6_upCsv_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.6a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Builder upstream/downstream COLUMNS file: ", svalue(gp_10.6_upCsv_btn))
    }
  )
  
  # browse to select builder upstream csv file
  lyt10.6a[9,1] <- "Additional upstream/downstream catchments table (.csv)"
  lyt10.6a[9,2] <- gp_10.6_upAddCsv_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.6a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Builder upstream/downstream columns file: ", svalue(gp_10.6_upAddCsv_btn))
    }
  )
  
  lyt10.6a[10,1:2] <- gseparator(horizontal = TRUE, container = lyt10.6a, expand=TRUE)
  
  # check box for optional remove benchmark catchments
  lyt10.6a[11,1] <- gp_10.6_build_check <- gcheckbox(text = "Remove each networks catchments from its\nupstream/downstream area?", checked=FALSE, container = lyt10.6a)
  
  # browse to select builder ranker csv file
  lyt10.6a[12,1] <- "Builder ranker file (.csv; optional)"
  lyt10.6a[12,2] <- gp_10.6_buildCsv_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.6a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Builder ranker file: ", svalue(gp_10.6_buildCsv_btn))
    }
  )
  
  # browse to select additional builder  csv file
  lyt10.6a[13,1] <- "Additional benchmark catchments table (.csv; optional)"
  lyt10.6a[13,2] <- gp_10.6_buildAddCsv_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.6a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Additional benchmark catchments table: ", svalue(gp_10.6_buildAddCsv_btn))
    }
  )
  
  lyt10.6a[14,1:2] <- gseparator(horizontal = TRUE, container = lyt10.6a, expand=TRUE)
  
  
  # check box for optional drop catchments
  lyt10.6a[15,1] <- gp_10.6_drop_check <- gcheckbox(text = "Drop catchments from all upstream/downstream\nareas? (.csv; optional)", checked=FALSE, container = lyt10.6a)
  
  # browse to select drop catchments csv file
  lyt10.6a[15,2] <- gp_10.6_dropCatch_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.6a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Catchments to drop file: ", svalue(gp_10.6_dropCatch_btn))
    }
  )
  
  lyt10.6a[16,1:2] <- gseparator(horizontal = TRUE, container = lyt10.6a, expand=TRUE)
  
  # check box for upstream shapefiles
  lyt10.6a[17,1] <- gp_10.6_shp_check <- gcheckbox(text = "Save upstream/downstream polygons?", checked=FALSE, container = lyt10.6a)
  
  lyt10.6a[18,1] <- "Output polygons file (.shp; optional)"
  lyt10.6a[18,2] <- gp_10.6_shpFile_btn <- gfilebrowse( # file browser
    text = "", 
    type = "save", 
    quote = TRUE, 
    container = lyt10.6a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Output polygons file: ", svalue(gp_10.6_shpFile_btn))
    }
  )
  
  # browse to select catchments shp file
  lyt10.6a[19,1] <- "Catchments file (.shp; optional)"
  lyt10.6a[19,2] <- gp_10.6_catchShp_btn <- gfilebrowse( # file browser
    text = "", 
    type = "open", 
    quote = TRUE, 
    container = lyt10.6a, 
    handler = function(h,...){
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Catchments shp: ", svalue(gp_10.6_catchShp_btn))
    }
  )
  
  # button to run function
  lyt10.6a[20,1:2] <- gp_10.6a_run_btn <- gbutton(
    text = "Calculate hydrology",
    container = lyt10.6a,
    handler = function(h, ...){ 
      svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Working...")
      
      if(length(svalue(gp_10.6_upCsv_btn)) > 0){ upCatchPath_10.6 <- svalue(gp_10.6_upCsv_btn) } else{ upCatchPath_10.6 <- "" } # assign upstream builder file
      if(length(svalue(gp_10.6_upAddCsv_btn)) > 0){ upCatchAddPath_10.6 <- svalue(gp_10.6_upAddCsv_btn) } else{ upCatchAddPath_10.6 <- "" } # assign upstream additional file
      if(length(svalue(gp_10.6_upCsv_btn)) == 0 & length(svalue(gp_10.6_upAddCsv_btn)) == 0){stop("Builder upstream/downstream or Additional upstream/downstream file must be provided.")} # error if neither provided
      
      if(svalue(gp_10.6_build_check) == TRUE){ # if box ticked, assign provided inputs to builder ranker and additional benchmarks files. Error if neither provided.
        if(length(svalue(gp_10.6_buildCsv_btn)) > 0){ buildCsvPath_10.6 <- svalue(gp_10.6_buildCsv_btn) } else{ buildCsvPath_10.6 <- "" }
        if(length(svalue(gp_10.6_buildAddCsv_btn)) > 0){ buildCsvAddPath_10.6 <- svalue(gp_10.6_buildAddCsv_btn) } else{ buildCsvAddPath_10.6 <- "" }
        if(length(svalue(gp_10.6_buildCsv_btn)) == 0 & length(svalue(gp_10.6_buildAddCsv_btn)) == 0){stop("Builder ranker or Additional upstream file must be provided if box checked.")}
      } else{ # if box not ticked, inputs are blank
        buildCsvPath_10.6 <- ""
        buildCsvAddPath_10.6 <- ""
      }
      
      if(svalue(gp_10.6_drop_check) == TRUE){ dropCatchPath_10.6 <- svalue(gp_10.6_dropCatch_btn) } else{ dropCatchPath_10.6 <- "" } # if box ticked, assign drop file
      if(svalue(gp_10.6_drop_check) == TRUE & length(svalue(gp_10.6_dropCatch_btn)) == 0){stop("Box checked but no Catchments to drop file selected.")} # error if box ticked but nothing provided
      
      if(svalue(gp_10.6_shp_check) == TRUE){ # if box checked, assign inputs. Both must be provded or error is given
        if(length(svalue(gp_10.6_shpFile_btn)) == 0 | length(svalue(gp_10.6_catchShp_btn)) == 0){stop("Both Output polygons file and catchments shp file needed if box checked.")}
        savePath_10.6 <- svalue(gp_10.6_shpFile_btn)
        catchShpPath_10.6 <- svalue(gp_10.6_catchShp_btn)
        if(length(svalue(gp_1_proj_btn))==0){ # projection file needed if box ticked
          svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "No projection file selected")
          stop("No projection file selected")
        }
        prj_10.6 <- svalue(gp_1_proj_btn)
      } else{
        savePath_10.6 <- ""
        catchShpPath_10.6 <- ""
        prj_10.6 <- ""
      ...
      tryCatch({
        metricsHydro(
          netPath = svalue(gp_10.6_inshp_btn),
          idColNet = "networks",
          catchDbfPath = svalue(gp_10.6_catchDbf_btn),
          intactCol = svalue(gp_10.6_intactcol_txt),
          areaCol = svalue(gp_10.6_areacol_txt),
          newCol = svalue(gp_10.6_newCol_txt),
          
          upCatchPath = upCatchPath_10.6,
          upCatchAddPath = upCatchAddPath_10.6,
          buildCsvPath = buildCsvPath_10.6,
          buildCsvAddPath = buildCsvAddPath_10.6,
          dropCatchPath = dropCatchPath_10.6,
          savePath = savePath_10.6,
          catchShpPath = catchShpPath_10.6,
          prjFile = prj_10.6
        )
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Hydrology added")
      }, error = function(cond){
        message("Could not calculate hydrology")
        message(cond)
        svalue(txtOutput) <- paste0(svalue(txtOutput), "\n", "Could not calculate hydrology")
      }
      )
    }
  )
  
  lyt10.6[1:20, 3] <- gp_10.6_text <- gframe("Tool description", container = lyt10.6, expand = TRUE, pos = 1)
  lyt10.6b <- glayout(container = gp_10.6_text, spacing = 10)
  lyt10.6b[1,1] <- glabel(paste0("Each tool in the 'Add network attributes' tab adds new fields to the networks shapefile.
                                 It is recommended you keep a copy of the original file before adding new attributes.
                                 ----------"))
