...
gen_target_table <- function(mdr, ecoRast, ecoBnd, outFile) {
  ...
  ecoAsc <- raster(ecoBnd)
  ecoAsc[ecoAsc == 0] <- NA # If raster has zero values filling the extent, convert them to NA 
  cellsize <- res(ecoAsc)[1]
  cellarea <- cellsize * cellsize / 1000000
  bnd.area <- sum(table(values(ecoAsc))) * cellarea
